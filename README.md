...
**界面长这样**（左侧导航 + 右侧内容，10 个页面）：

| 页面 | 能看到什么 |
| --- | --- |
| 总览 | 统计卡片（角色 / 事件 / 关系 / 未闭合因果 / 摘要 / 伏笔 / 物品 / 地点）· 世界门禁 · 最近事件 · 「有 N 条因果可提交」提醒 |
| 剧情摘要 | 场景 / 章节 / 卷摘要，带楼数范围与未解决项 |
| 角色档案 | 角色列表 → 详情：人格锚点（核心特质 / 说话方式 / 禁用词 / 口头禅 / 禁忌 / 小动作 / 决策模式 / **不可变外观**）、当前状态、**认知边界**（知道的世界 / 常识 / 无法理解的概念）、人设漂移值 |
| 人际关系 | 六维条形图（信任 / 好感 / 敬重 / 敌意 / 恐惧 / 人情债 + 趋势箭头）、**关系账本时间线**、永久锚点、知道 / 误以为 / 不知道 |
| 因果链 | 未闭合草稿（前因 → 后果 + 闭合判定）、因果边（上游 → 下游）、事件账本（已撤销的划掉） |
| 世界设定 | 世界门禁全部字段（力量上限 / 时间比例 / 禁止 / 允许 / 冻结）+ 地点 + 势力 |
| 物品追踪 | 归属 / 持有者 / 能力 / 代价 / 来历 / 跨世界状态 |
| 目标与伏笔 | 主线支线目标（线索 / 死路 / 期限 / 失败后果）+ 未回收伏笔 |
| 设置 | 世界 ID、预算、启用开关；**连接细节收在「高级」折叠里** |
| 调试 | 注入详情 · 队列 · 日志 · 错误日志 |

> **连接是隐形的**：地址自动探测（多端口并行，含 8080 / 8001 / 5000），失败后**后台静默重连**
> （5s→60s 退避），不需要你填地址、也不用点「重连」。状态栏只说「就绪 / 未就绪」。
>
> 结构与皮肤分离：`ui/panel.html`（骨架）+ `style.css`（`--mp-*` 变量换肤）+ `views.js`（数据 → HTML）。
> **想自己设计 UI 看 [`frontend/UI.md`](frontend/UI.md)** —— 换色只改 CSS 变量；也可以开 headless 模式
> （只注入不渲染）完全自绘。
>
> 万一 `ui/panel.html` 取不到（CSP / 路径异常），`index.js` 里内置了同款兜底 HTML，不会白屏。
